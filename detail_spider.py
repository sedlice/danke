...
def run_spider():
    myfun = MyFunc.MyFunc()
    while True:
        result_set = myfun.execute_pgsql_sql('delete from danke_waiting_data where id in (select id from danke_waiting_data order by id limit 1) returning dk_id,dk_url,dk_from,batch_time')
        if len(result_set) > 0:
